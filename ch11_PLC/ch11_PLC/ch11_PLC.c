...
char * find_the_word(char * sentence_ptr, char * searchWord_ptr, int * errorCode_ptr)
{
	char *returnValue_ptr = NULL; // declare a return pointer
	int sentence_length = 0; // declare length variable for sentence pointer
	int search_length = 0; // declare length variable for search pointer
	int i = 0; // declare iterator
	int x = 0; // declare iterator
	char testChar1 = 0; // DEBUGGING
	char testChar2 = 0; // DEBUGGING

	if (!sentence_ptr ) //check if sentence pointer is null
	{
		returnValue_ptr = NULL;
		*errorCode_ptr = ERROR_NULL_SENTENCE_POINTER;
	}
	else if (!searchWord_ptr) //check if search pointer is null
	{
		returnValue_ptr = NULL;
		*errorCode_ptr = ERROR_NULL_SEARCH_POINTER;
	}
	else if (!errorCode_ptr) //check if error code pointer is null
	{
		returnValue_ptr = NULL;
	}
	else
	{
		sentence_length = strlen(sentence_ptr); //set sentence length
		search_length = strlen(searchWord_ptr); //set search length
		if (search_length > sentence_length) // check if search is longer than sentence. If so, finding search is impossible
		{
			returnValue_ptr = NULL;
		...
		{
			*errorCode_ptr = ERROR_SEARCH_NOT_FOUND;
			for (i = 0; i <= (sentence_length - search_length); i++) //start loop to check if search is in sentence
			{
				testChar1 = *(sentence_ptr + i);
				testChar2 = *(searchWord_ptr);
				if (*(sentence_ptr + i) == *searchWord_ptr)
				{
					if (search_length == 1) //first char of search found. If search is only one char, call it done
					{
					...
					{
						for (x = 1; x < search_length; x++) //first char found, continue looking for rest of search
						{
							if (*(sentence_ptr + i + x) != *(searchWord_ptr + x)) //if next char isn't part of search, break loop and go to outer loop to continue search
							{
								break;
							}
							else if (x == (search_length - 1))
							{
								returnValue_ptr = (sentence_ptr + i);
								*errorCode_ptr = ERROR_CODE_SUCCESS;
								return returnValue_ptr;
