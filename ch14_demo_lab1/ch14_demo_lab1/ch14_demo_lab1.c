...
char * split_the_string(char * string_ptr, char delimiter)
{
	if (!string_ptr) //check if string_ptr is NULL
	{
		return ERROR_NULL_POINTER;
	}
	if (!delimiter) //check if delimiter is NULL
	{
		return ERROR_NULL_DELIMITER;
	}
	int count = 0; //declare count variable to count delimiters found
	char *new_ptr = NULL; //declare pointer for return value
	int string_length = strlen(string_ptr); //declare variable for pointer string length
	int position = 0; //declare variable for delimiter position

					  //loop through string checking to see if any of the characters are equal to the delimiter
	for (int i = 0; i < string_length; i++)
	{
		if (*(string_ptr + i) == delimiter) //check if the pointer position equals the delimiter
		{
			new_ptr = (string_ptr + i + 1); //set new pointer variable to delimiter position plus 1 for null
			count++; //increase delimiter count
			position = i; //track delimiter position
		}
	}

	if (count > 1) //check if more than one occurrence of delimiter
	{
		return ERROR_ABUNDANT_DELIMITER;
	}
	else if (count == 0) //if there's no delimiters, return the original pointer
	{
		return string_ptr;
	}
	else
	{
		*(string_ptr + position) = '\0'; //set the delimiter position to NULL
