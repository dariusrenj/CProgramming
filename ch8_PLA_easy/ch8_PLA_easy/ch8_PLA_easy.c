/*
Name: Zackery Vering
Title: ch8 Performance Lab A easy
Date: 18 July 2018
*/
...
int replace_char(char * buffer, char findChar, char replaceChar)
{
	...
	//check if the character being replaced and the character replacing it are the same
	...
	if (buffer)
	{
		...
		{
			if (*buffer == findChar)				// Checks for character replaced
			{
				count++;						// Increment the number of characters replaced
				*buffer =  replaceChar;					// Changes character to replacement character
			}
